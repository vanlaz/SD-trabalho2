...
export class AppComponent {
  ...
  ngOnInit() {
    this.httpClient.get('http://127.0.0.1:5002/servidor/tamanho/lista/'+ '0').subscribe(data => {
   ...
   mostrarViagemCompativel(){
    if(this.page == 4){
     ...
     var nome = this.motoristaInfo.toString().substring(this.motoristaInfo.indexOf('\"'), this.motoristaInfo.indexOf('\",'))
     var contato = this.motoristaInfo.toString().substring(this.motoristaInfo.indexOf('\",'), this.motoristaInfo.indexOf('\"]'))
  ...
  minhasViagens() {
    this.httpClient.get('http://127.0.0.1:5002/servidor/consulta/viagens/' + this.IdUser  + '/0').subscribe(data => {
  ...
  interesseEmCarona(origem:any, destino:any, data:any) {
    data = String(data).replace("/", "" )
    data = String(data).replace("/", "" )
    if(this.IdUser){
      this.httpClient.post('http://127.0.0.1:5002/servidor/interesse/motorista/' + this.IdUser + '/' + origem + '/'+ destino + '/'+ data, "").subscribe(response => {
  ...
  removeInteresseEmCarona(idCorrida:any) {
    this.httpClient.delete('http://127.0.0.1:5002/servidor/remove/interesse/motorista/' + idCorrida).subscribe(data => {
      this.listadeViagens = data as JSON;
      console.log(this.listadeViagens)
    })
  }
